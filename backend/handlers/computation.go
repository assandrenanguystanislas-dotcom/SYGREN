...
// === Fix E : recompute + backfill des moyennes précalculées ===

// recomputeStudentSessionResult recalcule la moyenne d'un élève pour une session
// et upsert la ligne dans student_session_results. Appelée par les hooks de
// saisie/suppression de notes (UpsertGrade, BulkUpsertGrades, DeleteGrade).
// Réutilise computeSessionResults (qui gère coefficients + exemptions) puis
// extrait le résultat de L'ÉLÈVE concerné.
func recomputeStudentSessionResult(studentID, sessionID string) error {
	results, err := computeSessionResults(sessionID)
	if err != nil {
		// Session supprimée ou inaccessible → on nettoie la ligne si elle existe.
		database.DB.Where("student_id = ? AND session_id = ?", studentID, sessionID).
			Delete(&models.StudentSessionResult{})
		return nil // non-fatal
	}
	// Cherche l'élève dans les résultats.
	for _, r := range results.Results {
		if r.StudentID == studentID {
			// Upsert : delete + create (simple, évite les contraintes unique).
			database.DB.Where("student_id = ? AND session_id = ?", studentID, sessionID).
				Delete(&models.StudentSessionResult{})
			ssr := models.StudentSessionResult{
				StudentID:    studentID,
				SessionID:    sessionID,
				ClassID:      r.ClassID,
				ClassLevel:   r.ClassLevel,
				Average:      r.Average,
				AverageScale: r.AverageScale,
				HasAverage:   r.HasAverage,
			}
			return database.DB.Create(&ssr).Error
		}
	}
	// Élève non dans les résultats (exempté ou sans note) → nettoie la ligne.
	database.DB.Where("student_id = ? AND session_id = ?", studentID, sessionID).
		Delete(&models.StudentSessionResult{})
	return nil
}

// recomputeSessionResults recalcule les moyennes de TOUS les élèves d'une session
// (batch). Utilisé par le backfill au démarrage.
func recomputeSessionResults(sessionID string) error {
	results, err := computeSessionResults(sessionID)
	if err != nil {
		return err
	}
	// Nettoie les anciennes lignes pour cette session (remplacement complet).
	database.DB.Where("session_id = ?", sessionID).Delete(&models.StudentSessionResult{})
	if len(results.Results) == 0 {
		return nil
	}
	batch := make([]models.StudentSessionResult, 0, len(results.Results))
	for _, r := range results.Results {
		batch = append(batch, models.StudentSessionResult{
	...
	return database.DB.CreateInBatches(batch, 100).Error
}

// BackfillStudentSessionResults reconstruit la table student_session_results
// au démarrage si elle est vide. Itère toutes les sessions validées/closed et
// recalcule les moyennes de leurs élèves (batch par session).
func BackfillStudentSessionResults() {
	var count int64
	database.DB.Model(&models.StudentSessionResult{}).Count(&count)
	if count > 0 {
		log.Println("[backfill] student_session_results déjà peuplée (", count, "lignes) — skip")
		return
	}
	var sessions []models.EvaluationSession
	// On backfill les sessions qui ont des notes (status closed/validated,
	// pas draft/cancelled/archived qui n'ont pas de notes valides).
	database.DB.Where("status IN ?", []string{"closed", "validated", "open"}).
		Find(&sessions)
	log.Println("[backfill] démarrage : ", len(sessions), " sessions à recalculer")
	for _, s := range sessions {
		if err := recomputeSessionResults(s.ID); err != nil {
			log.Printf("[backfill] erreur session %s : %v", s.ID, err)
		}
	}
	var final int64
	database.DB.Model(&models.StudentSessionResult{}).Count(&final)
	log.Println("[backfill] terminé : ", final, " lignes dans student_session_results")
}
