...
// CreateSession creates a new monthly evaluation session.
func CreateSession(w http.ResponseWriter, r *http.Request) {
	...
	// Vérifier l'unicité (une session par classe/mois/année)
	var count int64
	database.DB.Model(&models.EvaluationSession{}).
		Where("class_id = ? AND month = ? AND year = ?", req.ClassID, req.Month, req.Year).
		Count(&count)
	if count > 0 {
		middleware.JSONError(w, "une session existe déjà pour cette classe/mois/année", http.StatusConflict)
		return
	}
