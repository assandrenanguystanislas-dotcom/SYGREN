...
//
// GetPublicInfos retourne les informations affichées dans l'en-tête du
// tableau de bord pour TOUT utilisateur connecté :
//   - infos.admin_phone : téléphone de l'administrateur (haut à gauche)
//   - infos.sygren      : annonces de la bande défilante (haut à droite)
//
// Contrairement à GET /api/settings (réservé au module « settings »), cet
// endpoint n'exige AUCUN module : l'en-tête est visible de tous les rôles
// (admin, inspecteur, directeur, enseignant, parent, conseiller). Les
// valeurs restent MODIFIABLES uniquement via PUT /api/settings/{key} (admin).
func GetPublicInfos(w http.ResponseWriter, r *http.Request) {
        m := GetSettingsMap()
        jsonResponse(w, http.StatusOK, map[string]string{
                "admin_phone": m["infos.admin_phone"],
                "infos":       m["infos.sygren"],
        })
}
