...
func New(cfg *config.Config) http.Handler {
	...
	r.Group(func(r chi.Router) {
		...
		// Liste des candidats d'une classe (document officiel
		// « LISTE DES CANDIDATS ... A L'EXAMEN DU CEPE » du module
		// Élèves — lecture, RBAC de périmètre dans le handler).
		r.Get("/api/classes/{id}/candidates", handlers.GetClassCandidates)
		r.Group(func(r chi.Router) {
		...
		// Bande « Infos SYGREN » de l'en-tête — téléphone admin +
		// annonces défilantes. Accessible à TOUT utilisateur connecté
		// (aucun module requis — l'en-tête est visible de tous les
		// rôles ; cf. handlers.GetPublicInfos).
		r.Get("/api/public/infos", handlers.GetPublicInfos)
