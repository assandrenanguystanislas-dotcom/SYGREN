...
func UpsertGrade(w http.ResponseWriter, r *http.Request) {
	...
	if req.Value < 0 || req.Value > 20 {
		middleware.JSONError(w, "la note doit être comprise entre 0 et 20", http.StatusBadRequest)
		return
	}
...
func BulkUpsertGrades(w http.ResponseWriter, r *http.Request) {
	...
	for _, g := range req.Grades {
		...
		if g.Value < 0 || g.Value > 20 {
			middleware.JSONError(w, fmt.Sprintf("note invalide : %v (doit être 0-20)", g.Value), http.StatusBadRequest)
			return
		}
	}
