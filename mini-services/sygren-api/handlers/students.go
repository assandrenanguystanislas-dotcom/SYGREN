...
// === Students — Gestion des élèves avec matricule unique ===
// Accès :
...
// Format : SYG-AAAA-CLASSE-SEQ (ex: SYG-2024-CP1-001)
// (cahier des charges §3 Module 1 — attribution automatique)
func generateMatricule(classID string) (string, error) {
	var cls models.Class
	if err := database.DB.First(&cls, "id = ?", classID).Error; err != nil {
		return "", fmt.Errorf("classe introuvable")
	}
	year := time.Now().Year()
	// Compter le nombre d'élèves déjà dans cette classe
	var count int64
	database.DB.Model(&models.Student{}).Where("class_id = ?", classID).Count(&count)
	seq := count + 1
	return fmt.Sprintf("SYG-%d-%s-%03d", year, cls.Name, seq), nil
}

// CreateStudent creates a new student with auto-generated matricule.
func CreateStudent(w http.ResponseWriter, r *http.Request) {
	...
	matricule, err := generateMatricule(req.ClassID)
	if err != nil {
